...
Docling và model embedding được tách thành optional extras. Cấu hình mặc định là strict:
application kiểm tra model/dimension khi startup và ingestion thất bại nếu model không khả dụng
hoặc dimension không khớp.
...
Core-only vẫn parse TXT/CSV và image assets. PDF cần Docling. Nếu chưa cài model embedding,
chỉ chạy ingestion với `create_embeddings=false` hoặc bật fallback một cách tường minh cho
development:
